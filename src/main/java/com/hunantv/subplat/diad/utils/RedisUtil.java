...
public class RedisUtil  {
    ...
    private static JedisPool getPool(String ip,int port) {
        ...
        if(!maps.containsKey(key)) {
            ...
            //config.setMaxActive(DiadConfig.getInstance().getRedisMaxActive());
            ...
            try{  
                 ...
                 */
                pool = new JedisPool(config, ip, port, ProjectConfig.getInstance().getRedisTimeout());
                maps.put(key, pool);
            } catch(Exception e) {
                e.printStackTrace();
            }
	...
	public Jedis getJedis(String ip,int port) {
		...
		do{
    		...
    		count++;
		}while(jedis==null&&count<ProjectConfig.getInstance().getRedisRetryNum());
		return jedis;
	}
